...
public class DoorOfBurySystem
{
    ...
    {
        Kill = 0, // 自己参与杀死Boss，Boss由自己杀死
        Help = 1, // 自己参与杀死Boss，Boss不是由自己杀死
        None = 2, // 自己没有参与杀死Boss
    }
    ...
    public class DoorOfBurryData
    {
        ...
        public int bossMode { get; set; }//1恶魔，2邪神
        ...
        public string battleInfo
        {
            get
            {
                ...
                {
                    case 0:
                        return LanguageData.GetContent(47104);// 开启->"战斗中！"
                    case 1:
                        return LanguageData.GetContent(47105);// 完成->"已结束！"
                    case 2:
                        return LanguageData.GetContent(47105);// 超时->"已结束！"
                    default:
                        return LanguageData.GetContent(47106);// "未知"
                }
            }
        }
        // Boss名称
        public string bossName
        {
            ...
            {
                if (bossMode == 1)
                    return LanguageData.GetContent(47107);// "恶魔"
                else
                    return LanguageData.GetContent(47108);// "邪神"
            }
        }
    }
    ...
    public DoorOfBurySystem()
    {
        ...
        DOOR_NO_OPEN = LanguageData.GetContent(312); // 未解锁
        HAS_NO_DOOR = LanguageData.GetContent(47115); // 没有Boss->未发现恶魔
        HAS_DOOR_IN_CD = LanguageData.GetContent(1001109); // 有Boss->湮灭之门已开启
        HAS_DOOR_NO_CD = LanguageData.GetContent(47116); // 有Boss->可挑战恶魔
    ...
    public void OnDoorOfBuryCdResp(int cd, bool hasAnyDoor)
    {
        ...
        if (MogoWorld.thePlayer.level >= SystemRequestLevel.DOOROFBURY) // 已解锁 
        {
            SetUpCdUI();
        ...
        else // 未解锁
        {
            ...
            ChallengeUIViewManager.Instance.SetEndText((int)ChallengeGridID.DoorOfBury, DOOR_NO_OPEN); // 25级解锁
    ...
    private void AddDoorFx()
    {
        ...
        if (city == null) return;
        Transform door = city.transform.FindChild("transfer");
        if (door == null) return;
    ...
    private void OnEnterDoor()
    {
        ...
        //if (CD > 0)
        //{
        //    MogoMsgBox.Instance.ShowMsgBox("CD中!");
        //    return;
        //}
        //if (m_selectDoor.isWin)
        //{
        //    MogoMsgBox.Instance.ShowMsgBox("战斗已胜利！");
        //    return;
        //}
        //if (m_selectDoor.isLose)
        //{
        //    MogoMsgBox.Instance.ShowMsgBox("战斗已失败！");
        //    return;
        //}
        //todo 通知服务器
        if (m_currentCopyId < 0)
        {
            MogoMsgBox.Instance.ShowFloatingText(LanguageData.GetContent(47109));// "请选择湮灭之门"
        }
        else
        {
            ...
            MogoUIManager.Instance.ShowMogoDoorOfBuryUI(() =>
                {
                    if (DoorOfBuryUIViewManager.Instance != null)
                    {
                        ...
                        DoorOfBuryUIViewManager.Instance.SetDoorTitle(LanguageData.GetContent(47111));// "湮灭之门"
    ...
    private void OnDoorShowDataResp(List<DoorOfBurryData> doorList)
    {
        ...
        //{
        //    MogoMsgBox.Instance.ShowMsgBox("湮灭之门未开启！");
        //    return;
        //}
