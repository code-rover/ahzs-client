        ...
        private void PickDropResp(UInt32 errId, LuaTable items)
        {
            ...
            if (errId > 100)
            {
                ...
                if (Utils.ParseLuaTable(items, out data))
                {
                    ...
                    {
                        BillboardLogicManager.Instance.AddAloneBattleBillboard(MogoWorld.thePlayer.Transform.FindChild("slot_billboard").position, data[1], AloneBattleBillboardType.Gold);
                    }
                }
