...
class ActorDrop : ActorParent<EntityDrop>
{
    ...
    void Pick()
    {
        ...
        if (dropInfo != null)
        {
            if (dropInfo[4] > 0)
            {
                BillboardLogicManager.Instance.AddAloneBattleBillboard(MogoWorld.thePlayer.Transform.FindChild("slot_billboard").position, dropInfo[4], AloneBattleBillboardType.Gold);
            }
        }
