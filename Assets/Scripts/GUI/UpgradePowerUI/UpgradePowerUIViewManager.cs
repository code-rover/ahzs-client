...
public class UpgradePowerUIViewManager : MogoUIBehaviour 
{
    ...
    void Awake()
    {
        ...
        m_tranUpgradePowerUISystemList = m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUISystemList"]);
        m_camUpgradePowerUISystemListCamera = m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUISystemListCamera"]).GetComponentsInChildren<Camera>(true)[0];
        m_camUpgradePowerUISystemListCamera.GetComponent<UIViewport>().sourceCamera = GameObject.Find("Camera").GetComponent<Camera>();

        m_goUpgradePowerUITipEnableBtnBGDown = m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUITipEnableBtnBGDown"]).gameObject;
        m_lblUpgradePowerUITipEnableBtnText = m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUITipEnableBtnText"]).GetComponentsInChildren<UILabel>(true)[0];
        m_lblUpgradePowerUICurrentPowerNum = m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUICurrentPowerNum"]).GetComponentsInChildren<UILabel>(true)[0];

        m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUIBtnClose"]).gameObject.AddComponent<UpgradePowerUIButton>();
        m_myTransform.FindChild(m_widgetToFullName["UpgradePowerUITipEnableBtn"]).gameObject.AddComponent<UpgradePowerUIButton>();
