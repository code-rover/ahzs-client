...
public class CreateCharacterUIViewManager : MonoBehaviour
{
    ...
    public static CreateCharacterUIViewManager Instance
    {
        get
        {
            if (m_instance == null)
            {
                ...
                if (obj)
                {
                    m_instance = obj.transform.FindChild("CreateCharacterUI").GetComponentsInChildren<CreateCharacterUIViewManager>(true)[0];
                }
            }
    ...
    void Awake()
    {
        ...
        m_lblNewCharacterName = m_myTransform.FindChild(m_widgetToFullName["CreateCharacterUICharacterNameText"]).GetComponentsInChildren<UILabel>(true)[0];
