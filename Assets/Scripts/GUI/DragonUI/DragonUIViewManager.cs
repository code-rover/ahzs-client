...
public class DragonUIViewManager : MFUIUnit
{
    ...
    public override void CallWhenCreate()
    {
        ...
        uiCursor = GameObject.Find("MogoGlobleUIPanel").transform.FindChild("UICursor/UICursorSprite").GetComponentsInChildren<UISprite>(true)[0];
    ...
    void OnDragonUIDragBegin(int id)
    {
        for (int i = 0; i < m_listDragonItem.Count; ++i)
        {
            if (m_listDragonItem[i].name == id.ToString())
            {
                string spName = m_listDragonItem[i].transform.FindChild("DragonUIGridBG").GetComponentsInChildren<UISprite>(true)[0].spriteName;
    ...
    public void ShowFloatGoldAnim(int gridId)
    {
        for (int i = 0; i < m_listDragonItem.Count; ++i)
        {
            ...
            {
                GameObject goAnim = m_listDragonItem[i].transform.FindChild("DragonUIGridGoldAnim").gameObject;
    ...
    public void WishAddDragonItem(int level, string type, int gridId, string imgName = "", int color = 0)
    {
        ...
        AssetCacheMgr.GetUIInstance("DragonUIGridInfotest.prefab", (prefab, id, go) =>
        {
            ...
            obj.transform.FindChild("DragonUIGridLevel").GetComponentsInChildren<UILabel>(true)[0].text = "Lv" + level;
            obj.transform.FindChild("DragonUIGridName").GetComponentsInChildren<UILabel>(true)[0].text = type;
            UISprite s = obj.transform.FindChild("DragonUIGridBG").GetComponentsInChildren<UISprite>(true)[0];
    ...
    public void PickUpDragonItem(int gridId)
    {
        ...
        for (int i = 0; i < m_listDragonItem.Count; ++i)
        {
            if (m_listDragonItem[i].name == gridId.ToString())
            {
                ...
                TweenColor tc = m_listDragonItem[i].transform.FindChild("DragonUIGridAnimSlot/MogoUVAnimationWithShader").GetComponentsInChildren<TweenColor>(true)[0];
                tc.enabled = true;


                TweenAlpha ta = m_listDragonItem[i].transform.FindChild("DragonUIGridName").GetComponentsInChildren<TweenAlpha>(true)[0];
                ta.enabled = true;

                ta = m_listDragonItem[i].transform.FindChild("DragonUIGridLevel").GetComponentsInChildren<TweenAlpha>(true)[0];
                ta.enabled = true;
            }
        }
    }
    ...
    {
        m_listDragonItem[id].transform.FindChild("DragonUIGridToolTip").gameObject.SetActive(isShow);
    }

    public void SetDragonUIToolTipName(int id, string name)
    {
        m_listDragonItem[id].transform.FindChild(
    ...
    public override void CallWhenShow()
    {
        ...
        if (m_myTransform.FindChild("DragonIcon").GetComponentsInChildren<UITexture>(true)[0].mainTexture == null)
        {

            AssetCacheMgr.GetResourceAutoRelease("lyfw_ditu.png", (obj) =>
            {

                m_myTransform.FindChild("DragonIcon").GetComponentsInChildren<UITexture>(true)[0].mainTexture = (Texture)obj;
            });
        }
