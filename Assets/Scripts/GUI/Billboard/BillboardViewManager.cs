...
public class BillboardViewManager : MogoUIBehaviour
{
    ...
    void Start()
    {
       billboardList =  transform.FindChild("Anchor/MogoMainUIPanel/BillboardList").gameObject;
    }
    void OnUpdateBillboardPos(Vector3 position, uint playerid)
    {
        //这里transform对象在调用的时候已经销毁，需要确认调用与销毁时机
        ...
        if (!ViewCamera || !RelatedCamera)//在打包加载项目中有可能会空
            return;
    ...
    {
        TimerHeap.AddTimer(1, 0, () => { billboardList.SetActive(isShow); }); //有些地方会在trigger里面直接调用 u3d不允许这样做 为了兼容 延迟错开调用时间 by MaiFeo
    }
