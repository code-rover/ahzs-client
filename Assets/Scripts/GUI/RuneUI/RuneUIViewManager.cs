...
public class RuneUIViewManager : MFUIUnit
{
    ...
    {
        SetUIDirty();
        m_lblLifePower = m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetPowerNum"]).GetComponentsInChildren<UILabel>(true)[0];

        m_goRuneUICursor = m_myTransform.FindChild(m_widgetToFullName["RuneUICursorUVAnim"]).gameObject;
        m_goRuneUIPackage = m_myTransform.FindChild(m_widgetToFullName["RuneUIPackage"]).gameObject;

        for (int i = 0; i < 11; ++i)
        {
            m_arrInsetGridInfo[i] = m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetGridInfo" + i]).gameObject;
            m_arrInsetGridLock[i] = m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetGridLock" + i]).gameObject;
            m_arrInsetGird[i] = m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetGrid" + i]).gameObject.transform;

            m_listRuneInsetLimitSign.Add(m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetGrid" + i + "LimitSign"]).gameObject);
        }

        for (int i = 0; i < 16; ++i)
        {
            m_arrPackageGrid[i] = m_myTransform.FindChild(m_widgetToFullName["RuneUIPacakgeGridBG" + i]);
        }

        m_myTransform.FindChild(m_widgetToFullName["EquipCamera"]).GetComponentsInChildren<UIViewport>(true)[0].sourceCamera =
            GameObject.Find("MogoMainUI").transform.FindChild("Camera").GetComponentsInChildren<Camera>(true)[0];

        m_texInsetBodyBG = m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetBG"]).GetComponentsInChildren<UITexture>(true)[0];
   ...
   // void Awake()
    ...
    void OnRuneUIDragBegin(int id)
    {
        for (int i = 0; i < m_listRuneItem.Count; ++i)
        {
            if (m_listRuneItem[i].name == id.ToString())
            {
                ...
                string spName = m_listRuneItem[i].transform.FindChild("RuneUIInsetGridBG").GetComponentsInChildren<UISprite>(true)[0].spriteName;
    ...
    void OnRuneUIInsetDragBegin(int id)
    {
        for (int i = 0; i < m_listRuneInsetItem.Count; ++i)
        {
            ...
            {
                //RuneUIViewManager.Instance.ShowRuneUICursor(true);
                string spName = m_listRuneInsetItem[i].transform.FindChild("RuneUIInsetGridBG").GetComponentsInChildren<UISprite>(true)[0].spriteName;
                MogoGlobleUIManager.Instance.ShowUICursorSprite(spName);
                RuneUIViewManager.Instance.ShowRuneUIToolTip(i, false, false);
                break;
            }
    ...
    public void ShowRuneUIToolTip(int id, bool isShow, bool isPackage)
    {
        if (isPackage)
        {
            m_listRuneItem[id].transform.FindChild("RuneUIGridToolTip").gameObject.SetActive(isShow);
            m_listRuneItem[id].transform.FindChild("RuneUIGridToolTip").gameObject.transform.localPosition = 
                m_listRuneItem[id].transform.FindChild("RuneUIGridToolTipLeft").localPosition;
        }
        else
        {
            m_listRuneInsetItem[id].transform.FindChild("RuneUIGridToolTip").gameObject.SetActive(isShow);
            m_listRuneInsetItem[id].transform.FindChild("RuneUIGridToolTip").gameObject.transform.localPosition 
                = m_listRuneInsetItem[id].transform.FindChild("RuneUIGridToolTipRight").localPosition;
        }
    }

    public void SetDragonUIToolTipName(int id, string name)
    {
        m_listRuneItem[id].transform.FindChild(
    ...
    public void SetRuneUITooltipCurrentDesc(int id, string desc,bool isPackage)
    {
        ...
        {
            m_listRuneInsetItem[id].transform.FindChild(
                "RuneUIGridToolTip/RuneUIGridToolTipInfo/RuneUIGridToolTipInfoCurrentDetail").GetComponentsInChildren<UILabel>(true)[0].text = desc;
        }
    }
    ...
    public void LoadPlayerModel()
    {
        ...
        cam.position = obj.transform.FindChild("slot_camera").position + obj.transform.forward * 3.2f;
    ...
    {
        m_myTransform.FindChild(m_widgetToFullName["RuneUIInsetGridLevel" + id + "Text"]).GetComponentsInChildren<UILabel>(true)[0].text = level;
    }
