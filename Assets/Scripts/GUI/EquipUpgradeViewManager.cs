    ...
    public void ShowMainUI(bool isShow = true)
    {
        ...
        for (int i = 0; i < m_viewData.materialIdList.Count; i++)
        {
            ...
            AssetCacheMgr.GetUIInstance(MATERIAL_ICON_PREFAB, (str, id, obj) =>
            {
                ...
                UISprite bg = go.transform.FindChild("EquipUpgradeCostMaterialIconBG").GetComponent<UISprite>();
                UISprite fg = go.transform.FindChild("EquipUpgradeCostMaterialIconFG").GetComponent<UISprite>();
                UILabel lbl = go.transform.FindChild("EquipUpgradeCostMaterialIconNum").GetComponent<UILabel>();
