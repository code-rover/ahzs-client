...
public class MailDetailGrid : MonoBehaviour 
{
    ...
    void Awake()
    {
        ...
        m_lblMailDetailInfoText = m_myTransform.FindChild(m_widgetToFullName["MailDetailInfoText"]).GetComponentsInChildren<UILabel>(true)[0];
        m_lblMailDetailReciverName = m_myTransform.FindChild(m_widgetToFullName["MailDetailRecvierName"]).GetComponentsInChildren<UILabel>(true)[0];
        m_lblMailDetailSenderName = m_myTransform.FindChild(m_widgetToFullName["MailDetailSenderName"]).GetComponentsInChildren<UILabel>(true)[0];
        m_lblMailDetailTimeText = m_myTransform.FindChild(m_widgetToFullName["MailDetailTimeText"]).GetComponentsInChildren<UILabel>(true)[0];
        m_lblMailDetailTitleText = m_myTransform.FindChild(m_widgetToFullName["MailDetailTitleText"]).GetComponentsInChildren<UILabel>(true)[0];

        m_spMailItemGetSign = m_myTransform.FindChild(m_widgetToFullName["MailDetailItemGetSign"]).GetComponentsInChildren<UISprite>(true)[0];
        m_goOneKeyGetMailItem = m_myTransform.FindChild(m_widgetToFullName["MailDetailOneKeyGetBtn"]).gameObject;

        for (int i = 0; i < 5; ++i)
        {
            m_arrSpMailDetailItem[i] = m_myTransform.FindChild(m_widgetToFullName["MailDetailItem" + i + "FG"]).GetComponentsInChildren<UISprite>(true)[0];
            m_arrSpMailDetialItemBG[i] = m_myTransform.FindChild(m_widgetToFullName["MailDetailItem" + i + "BG"]).GetComponentsInChildren<UISprite>(true)[0];
            m_arrLblMailDetailItemNum[i] = m_myTransform.FindChild(m_widgetToFullName["MailDetailItem" + i + "Num"]).GetComponentsInChildren<UILabel>(true)[0];
        }
    }
