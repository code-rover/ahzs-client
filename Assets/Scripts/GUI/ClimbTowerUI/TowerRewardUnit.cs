...
public class TowerRewardUnit : MogoParentUI
{
    ...
    private void SetPicVisible(int id, bool show)
    {
        ...
        var trans = m_Transform.FindChild(string.Format("tower/{0}", id + 1));
        trans.localPosition = new Vector3(trans.localPosition.x, trans.localPosition.y, id + RewardID);
        m_Transform.FindChild(lPath).gameObject.SetActive(show);
        m_Transform.FindChild(rPath).gameObject.SetActive(show);
    }
    ...
    public bool Unlocked
    {
        set
        {
            if (value)
            {
                m_Transform.FindChild("tower/node").gameObject.SetActive(false);
            }
            else
            {
                m_Transform.FindChild("tower/node").gameObject.SetActive(true);
            }
        }
    }
    public bool HighLight
    {
        set
        {
            if (value)
            {
                ...
                m_Transform.FindChild("tower/TriPager").gameObject.SetActive(true);
                m_Transform.FindChild("boxReward/bg").GetComponentsInChildren<MogoTwoStatusButton>(true)[0].SetButtonDown(true);
            }
            else
            {
                m_Transform.FindChild("tower/TriPager").gameObject.SetActive(false);
                m_Transform.FindChild("boxReward/bg").GetComponentsInChildren<MogoTwoStatusButton>(true)[0].SetButtonDown(false);
            }
        }
    }
    public bool Grey
    {
        set
        {
            ...
            var left = m_Transform.FindChild(lPath).GetComponentsInChildren<UISprite>(true)[0];
            left.ShowAsWhiteBlack(value);
            var right = m_Transform.FindChild(rPath).GetComponentsInChildren<UISprite>(true)[0];
    ...
    void Awake()
    {
        ...
        m_alreadyGet = m_Transform.FindChild("boxReward/btnRewardBox/btnRewardBGDown").gameObject;
        m_notGet = m_Transform.FindChild("boxReward/btnRewardBox/btnRewardBGUp").gameObject;
        m_lblRewardName = m_Transform.FindChild("boxReward/bg/txtRewardName").GetComponentsInChildren<UILabel>(true)[0];
        m_Transform.FindChild("boxReward/btnRewardBox").GetComponentsInChildren<MogoButton>(true)[0].clickHandler += OnSingleButtonClicked;
        m_Transform.FindChild("tower/tipArea").GetComponentsInChildren<MogoButton>(true)[0].clickHandler += OnTowerTipClicked;
        lblTowerDesc = m_Transform.FindChild("tower/TriPager/node/lblCurrentLevel").GetComponentsInChildren<UILabel>(true)[0];
        m_towerDescObj = m_Transform.FindChild("tower/TriPager").gameObject;
        lblForce = m_Transform.FindChild("tower/node/force").GetComponent<UILabel>();
        lblTowerName = m_Transform.FindChild("tower/text").GetComponent<UILabel>();
        lblForceCurrent = m_Transform.FindChild("tower/TriPager/node/lblForce").GetComponent<UILabel>();
