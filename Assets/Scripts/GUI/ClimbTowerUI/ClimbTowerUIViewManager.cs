...
public class ClimbTowerUIViewManager : MogoParentUI
{
    ...
    void Start()
    {
        m_closeAnchor.uiCamera = GameObject.Find("MogoMainUI").transform.FindChild("Camera").GetComponentsInChildren<Camera>(true)[0];
        var m_rewardImg = m_transform.FindChild("report/img").gameObject;
        m_rewardImg.GetComponentsInChildren<UIViewport>(true)[0].sourceCamera = GameObject.Find("MogoMainUI").transform.FindChild("Camera").GetComponentsInChildren<Camera>(true)[0];
    }
    ...
    {
        m_rewardCamera = m_transform.FindChild("RightBox/RewardList/RewardListCamera");
        if (m_rewardTip == null)
        {
            m_rewardTip = m_transform.FindChild("RightBox/RewardTip").gameObject;
            m_rewardTip.AddComponent<RewardGridTip>().TipNum = 4;
        }
        if (m_bigRewardTip == null)
        {
            m_bigRewardTip = m_transform.FindChild("RightBox/BigRewardTip").gameObject;
