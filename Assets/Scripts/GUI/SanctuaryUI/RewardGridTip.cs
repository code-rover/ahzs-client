...
public class RewardGridTip : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < TipNum; ++i)
        {
            m_arrLblReward[i] = m_myTransform.FindChild("WeekRankDialogReward" + i + "/" + i + "Text").GetComponentsInChildren<UILabel>(true)[0];
        }

        m_lblTipTitle = m_myTransform.FindChild("WeekRankDialogRewardTitleText").GetComponentsInChildren<UILabel>(true)[0];
    }
