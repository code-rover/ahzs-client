...
public class FriendGridData
{
    ...
    public bool isKill = false; //【true = 胜利】自己参与杀死Boss，Boss由自己杀死
    public bool isHelp = false; //【true = 胜利】自己参与杀死Boss，Boss不是由自己杀死
    public bool isWin = false; //【true = 胜利】
    public bool isLose = false; //【true = 失败】
    public int hour;
    public int minus;
}
...
public class DoorOfBuryUIViewManager : MogoUIBehaviour
{
    ...
    void Initialize()
    {
        ...
        m_myTransform.FindChild(m_widgetToFullName["DoorOfBuryChooseEnterBtn"]).GetComponentsInChildren<MogoButton>(true)[0].clickHandler += OnEnterBtnUp;
        m_myTransform.FindChild(m_widgetToFullName["DoorOfBuryUICloseBtn"]).GetComponentsInChildren<MogoButton>(true)[0].clickHandler += OnCloseBtnUp;
    }
