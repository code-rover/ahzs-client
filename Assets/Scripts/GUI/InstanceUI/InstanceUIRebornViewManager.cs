...
public class InstanceUIRebornViewManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        m_instanceRebornItemList = m_myTransform.FindChild(m_widgetToFullName["InstanceRebornItemList"]);
    ...
    {
        m_listInstanceRebornItem[id].transform.FindChild("InstanceRewardItemFG").GetComponentsInChildren<UISprite>(true)[0].spriteName = imgName;
        m_listInstanceRebornItem[id].transform.FindChild("InstanceRewardItemText").GetComponentsInChildren<UILabel>(true)[0].text = itemName;
    }
    ...
    public void AddRebornUIItemList(List<int> itemId, List<string> itemName, Action act = null)
    {
        for (int i = 0; i < itemId.Count; ++i)
        {
            ...
            AssetCacheMgr.GetUIInstance("InstanceRewardItem.prefab", (prefab, guid, go) =>
            {
                ...
                UISprite spFG = obj.transform.FindChild("InstanceRewardItemFG").GetComponentsInChildren<UISprite>(true)[0];
                UISprite spBG = obj.transform.FindChild("InstanceRewardItemBG").GetComponentsInChildren<UISprite>(true)[0];
                UILabel lblText = obj.transform.FindChild("InstanceRewardItemText").GetComponentsInChildren<UILabel>(true)[0];

                obj.GetComponent<MyDragCamera>().RelatedCamera = m_myTransform.FindChild("InstanceRebornItemList/InstanceRebornItemListCamera").GetComponent<Camera>();
                ...
                if (index == itemId.Count - 1)
                {
                    if (m_listInstanceRebornItem.Count <= 5)
                    {
                        ...
                        m_myTransform.FindChild(m_widgetToFullName["InstanceRebornItemListCamera"]).localPosition =
                            m_myTransform.FindChild(m_widgetToFullName["InstancePassReborn" + itemId.Count + "ItemStartPos"]).localPosition;
                    }
                    else
                    {
                        m_myTransform.FindChild(m_widgetToFullName["InstanceRebornItemListCamera"]).localPosition =
