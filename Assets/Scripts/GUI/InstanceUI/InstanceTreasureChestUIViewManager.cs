...
public class InstanceTreasureChestUIViewManager : MogoUIBehaviour 
{
    ...
    public void SetRewardItemID(List<int> idList)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            if (i < idList.Count)
            {
                UISprite itemSprite = m_listInstaceRewardItem[i].transform.FindChild("InstanceTreasureChestUIRewardItem" + i + "FG").GetComponentsInChildren<UISprite>(true)[0];
                UISprite bgSprite = m_listInstaceRewardItem[i].transform.FindChild("InstanceTreasureChestUIRewardItem" + i + "BG").GetComponentsInChildren<UISprite>(true)[0];                
    ...
    public void SetInstanceLevelRewardItemImage(List<string> imageNameList, List<string> itemNameList)
    {
        for (int i = 0; i < 3; ++i)
        {
            if (i < imageNameList.Count)
            {
                ...
                m_listInstaceRewardItem[i].transform.FindChild("InstanceTreasureChestUIRewardItem" + i + "Text").GetComponentsInChildren<UILabel>(true)[0].text = itemNameList[i];
            }
            else
            {
                m_listInstaceRewardItem[i].SetActive(false);
    ...
    public void SetInstanceLevelRewardItemCount(List<int> countList)
    {
        for (int i = 0; i < 3; ++i)
        {
            ...
            {
                if (countList[i] > 1)
                    m_listInstaceRewardItem[i].transform.FindChild("InstanceTreasureChestUIRewardItem" + i + "Count").GetComponentsInChildren<UILabel>(true)[0].text = countList[i].ToString();
                else
                    m_listInstaceRewardItem[i].transform.FindChild("InstanceTreasureChestUIRewardItem" + i + "Count").GetComponentsInChildren<UILabel>(true)[0].text = "";
            }          
        }
    }
