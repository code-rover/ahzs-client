...
public class MogoFXManager
{
    ...
    /// <param name="RelatedCamera">参照的相机</param>
        ...
        float xOffset = 0, float yOffset = 0, float zOffset = 0, Action action = null)
    {
        ...
        //uiMainCam.enabled = false;
        //uiMainCam.eventReceiverMask = 0;
        //TimerHeap.AddTimer(1000, 0, () => { uiMainCam.enabled = true; });//飞完才true
    ...
    public void RemoveEnemyInScreen(uint id)//此方法待优化，换一种处理形式，4个for循环太奇葩了
    {
    ...
    public void UpdateEnemyInScreen()//此方法待优化，那堆if else没法看
    {
        if (m_listEnemyObj.Count == 0 || GetBillboardCamera() == null)
            return;

        float screenWidth = GetBillboardCamera().GetScreenWidth();
        float screenHeight = GetBillboardCamera().GetScreenHeight();
        ...
        for (int i = 0; i < m_listEnemyScreenObj.Count; ++i)
        {
            ...
            if (m_listEnemyObj.Count > i && m_listEnemyObj[i] != null && m_listEnemyScreenObj[i] != null && Camera.mainCamera != null)
            {
                //LoggerHelper.Debug(m_listEnemyScreenObj[i].name + " In");
                Vector3 pos = Camera.mainCamera.WorldToScreenPoint(m_listEnemyObj[i].transform.position);
                Vector3 oriPos = Camera.mainCamera.WorldToScreenPoint(MogoWorld.thePlayer.Transform.position);
    ...
    public void AttachUpgrateFX()
    {
        AssetCacheMgr.GetUIInstance("fx_renwu_shengji.prefab", (prefab, id, obj) =>
        {
            ...
            go.transform.parent = MogoWorld.thePlayer.GameObject.transform.FindChild("slot_billboard");//此方法待优化，将slot_billboard获取方法放到EntityMySelf里
    ...
    public void SceneCameraTurnGray()
    {
        ...
        if (Camera.mainCamera.GetComponent<MogoGray>() == null)
        {
            AssetCacheMgr.GetUIResource("MogoGray.mat", (obj) =>
            {
                Camera.mainCamera.gameObject.AddComponent<MogoGray>().Mat = (Material)obj;
                Camera.mainCamera.GetComponent<MogoGray>().enabled = true;
            });
        }
        else
        {
    ...
    public void SceneCameraTurnColor()
    {
        ...
        Camera.mainCamera.GetComponent<MogoGray>().enabled = false;
    }
