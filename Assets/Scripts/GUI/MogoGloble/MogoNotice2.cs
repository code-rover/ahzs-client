...
public class MogoNotice2 : MonoBehaviour
{
    ...
    {
        return transform.FindChild(m_widgetToFullName[str]);
    }
    ...
    private void LoadNotice()
    {
        ...
        for (int i = 0; i < m_currentNoticeList.Count; i++)
        {
            ...
            AssetCacheMgr.GetUIInstance(LIST_GRID_PREFAB, (str, id, obj) =>
            {
                ...
                UILabel label = go.transform.FindChild("NoticeGridText").GetComponent<UILabel>();
                GameObject goIsNew = go.transform.FindChild("NoticeGridIsNew").gameObject;
