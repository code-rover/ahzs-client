...
public class TimeLimitActivityInfo : MogoUIBehaviour
{
    ...
    /// <summary>
    /// 限时活动详细信息-在父节点Active = false时设置信息,故自己调用Awake
    /// </summary>
            ...
            if (m_arrItem[0] != null)
            {
                for (int i = 0; i < 4; ++i)
                {
                    if (i < count)
                    {
                        ...
                        UISprite spFG = m_arrItem[i].transform.FindChild("TimeLimitActivityUIItem" + i + "FG").GetComponentsInChildren<UISprite>(true)[0];
                        UISprite spBG = m_arrItem[i].transform.FindChild("TimeLimitActivityUIItem" + i + "BG").GetComponentsInChildren<UISprite>(true)[0];
