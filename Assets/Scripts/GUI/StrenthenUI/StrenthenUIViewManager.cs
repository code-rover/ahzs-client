...
public class StrenthenUIViewManager : MogoUIBehaviour
{
    ...
    private const int ICONGRIDNUM = 10; // 总共10个格子
    ...
    private const float WAITINGTIME = 0.5f; // 强化按钮不能连续点击，间隔500ms
    ...
    /// 强化属性界面强化等级
    /// </summary>
    /// <param name="strenthenStep">阶数</param>
    /// <param name="progress">级数进度</param>
    ...
    {
        UISprite sp = m_listEquipmentGrid[gridId].transform.FindChild("StrenthenDialogIconGridImg").GetComponentsInChildren<UISprite>(true)[0];
    ...
    {
        m_listEquipmentGrid[gridId].transform.FindChild("StrenthenDialogIconGridTextImg").GetComponentsInChildren<UISprite>(true)[0].spriteName = imgName;
    }

    public void ShowEquipmentUpSign(int gridId, bool show = true)
    {
        m_listEquipmentGrid[gridId].transform.FindChild("StrenthenDialogIconGridUp").gameObject.SetActive(show);
    }
    ...
    private void OnBaseReward2TextAnimationEnd()
    {
        ...
        m_lblBaseReward0TextAnimation.GetComponentsInChildren<TweenScale>(true)[0].Reset(); // 重置Position
        m_lblBaseReward1TextAnimation.GetComponentsInChildren<TweenScale>(true)[0].Reset(); // 重置Position
        m_lblBaseReward2TextAnimation.GetComponentsInChildren<TweenScale>(true)[0].Reset(); // 重置Position
    ...
    /// 设置强化按钮
    /// </summary>
    /// <param name="isEnable">是否变灰</param>
    ...
    private UILabel GetStrenthenEquipmentGridText(int tab)
    {
        ...
        if (tab >= 0 && tab < m_listEquipmentGrid.Count)
        {
            ...
            {
                Transform tranGridText = goEquipmentGrid.transform.FindChild("StrenthenDialogIconGridText");
