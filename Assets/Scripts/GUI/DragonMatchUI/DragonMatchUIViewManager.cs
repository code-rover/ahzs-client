...
public class DragonMatchUIViewManager : MogoUIBehaviour
{
        ...
        if (m_tranDragonMatchUIRoundDOTList == null)
        {
            if (!IsDragonMatchUIRoundDOTListLoaded)
            {
                ...
                AssetCacheMgr.GetUIInstance("DragonMatchUIRoundDOTList.prefab", (prefab, guid, go) =>
                {
                    ...
                    {
                        Transform tranDOT = m_tranDragonMatchUIRoundDOTList.FindChild(string.Concat("DragonMatchUIRoundDOT", round));
                        m_listRoundDOTFG.Add(tranDOT.FindChild("DragonMatchUIRoundDOTFG").gameObject);
                    }

                    Transform tranDOTLine = m_tranDragonMatchUIRoundDOTList.FindChild("DragonMatchUIRoundDOTLine");
                    for (int round = 0; round < 10; round++)
                    {
                        m_listRoundDOTLine.Add(tranDOTLine.FindChild(string.Concat("DragonMatchUIRoundDOT", round, "Line")).gameObject);
    ...
    public void LoadDragonMatchScene(Action action = null)
    {
        if (!IsDragonMatchSceneLoaded)
        {
            ...
            AssetCacheMgr.GetInstance("fly.prefab", (prefab, guid, go) =>
            {
                ...
                m_camScene = obj.transform.FindChild("Main Camera").GetComponentsInChildren<Camera>(true)[0];
                ...
                {
                    GameObject goDragon = obj.transform.FindChild("NPC_2061_0" + i).gameObject;
    ...
    private void SetDragonUIInfo(int theLoadDragonIndex)
    {
        if (m_maplistDragonPlayerInfoUI.ContainsKey(theLoadDragonIndex))
        {
            ...
            Transform tranName = m_listDragonParent[theLoadDragonIndex].transform.FindChild("slot_billboard");
        }      
    }
