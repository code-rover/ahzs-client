...
public class InsetUIViewManager : MogoUIBehaviour
{
    ...
    public void SetEquipmentIcon(int gridId, string imgName, int color = 0)
    {
        ...
        MogoUtils.SetImageColor(m_listEquipmentGrid[gridId].transform.FindChild("StrenthenDialogIconGridImg").GetComponentsInChildren<UISprite>(true)[0], color);
    }
    public void SetEquipmentIconBG(int gridId, string imgName)
    {
        UISprite sp = m_listEquipmentGrid[gridId].transform.FindChild("StrenthenDialogIconGridImgBG").GetComponentsInChildren<UISprite>(true)[0];
        sp.spriteName = imgName;
    }
